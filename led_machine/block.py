...
class BlockSetting(AlterPixelSetting):
    ...
    def _get_color(self, pixel: int) -> Optional[ColorAlias]:
        offset = 0
    ...
    def alter(self, seconds: float, list_index: int, pixel_index: int, pixels,
        ...
        low_pixel_color: Optional[ColorAlias] = self._get_color(low_pixel) or pixel_color
        high_pixel_color: Optional[ColorAlias] = self._get_color(high_pixel) or pixel_color
